...
const queryDOMElement = (selector) => document.querySelector(selector)
const getDOMElementById = (id) => document.getElementById(id)
  ...
  if (square.matches(".square")) {
    ...
    if (isValidMove(square, color)) {
      ...
      if (isValidMoveAvailable(nextColor)) {
        ...
        log(`%c turn not swithced, \t color: ${color}, nextColor: ${nextColor}`, "color: crimson")
      }
...
themes.addEventListener('click', displayThemes)
...
coordsCheckbox.addEventListener('change', handleCoords)
possibleMovesCheckbox.addEventListener('change', helperDotsHandler)
